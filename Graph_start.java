...
public class Graph_start {
	...
	public void newEdge(String K1, String K2, int wert) {
		...
		if (n1 != -1 && n2 != -1 && kante[n2][n1] == 0 && kante[n1][n2] == 0)
			...
			System.out.println("Keine Speicherung m�glich, da bereits eine Verbindung der Knoten existiert.");
	}
	...
	public void deleteNode(int Kn) {		
		...
		for (int i = 0; i < knzahl; i++) {
			if (knoten[i] == null) {
				//Aufr�cken der Knoten, damit keine L�cke im Array bleibt
				for (int j = i; j < knzahl; j++) {
					knoten [j] = knoten [j+1];
				}
				//Aufr�cken der Kantenmatrix, damit keine L�cke im Array bleibt
